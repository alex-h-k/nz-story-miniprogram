...
export default function TripForm() {
  ...
  const [form, setForm] = useState({
    ...
    companionPref: '',      // 'any' | 'male_only' | 'female_only' | 'rainbow_friendly'  (希望和, solo only)
  ...
  const handleNext = () => {
    ...
    if (step === 1 && form.groupType === 'solo' && form.groupingPref === 'join_group' && !form.groupIdentity) {
      Taro.showToast({ title: '请选择你的性别', icon: 'none' })
      return
    }
    ...
    if (step === 1 && form.groupType === 'solo' && form.groupingPref === 'join_group' && !form.companionPref) {
